...
# ------------------------------------------------------------------------------
# Retrieve per-inverter data from the Hypon Cloud Platform
#
# Arguments
#  $1 The authentication token to use
# ------------------------------------------------------------------------------
retrieveInverterData () {
  local authToken=${1}
  local system_id
  local dataUrl
  local dataRequest
  ...
  system_id=$(bashio::config 'system_id')
  dataUrl="$HYPON_URL/plant/$system_id/inverter?page=1"

  bashio::log.info "Load Inverter Data Start"

  dataRequest=$(hypon_request_json_with_backoff "Load inverter data" "$dataUrl" \
    -H "$ACCEPT_HEADER" \
    -H 'User-Agent: Mozilla/5.0' \
    -H "authorization: Bearer $authToken")

  bashio::log.info "Load Inverter Data End"
  echo "$dataRequest"
}
